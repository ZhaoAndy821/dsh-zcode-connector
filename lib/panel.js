...
function renderRuns(s) {
  ...
  const currentIsAgent = current && current.agentType;
  ...
  // A main-session turn with no subagent of its own still deserves a live line.
  if (live.generating && current && !currentIsAgent && running.length === 0) {
    details.push('<div class="indent" style="margin-top:6px">' +
      '<div>主会话正在推理</div>' +
      '<div class="doing">第 ' + ((current.iteration || 0) + 1) + ' 轮 · 已 ' + dur(current.waitingMs) +
      (current.phase ? ' · ' + esc(phase(current.phase)) : '') +
      (current.tool ? ' · 上一个工具 ' + esc(current.tool) : '') + '</div>' +
      '<div class="meta" style="color:var(--muted);font-size:11px">本会话工具调用 ' + (current.toolCalls || 0) + '</div>' +
      '</div>');
  }
  ...
  for (const task of tasks) {
    ...
    for (const child of children) {
      ...
      rows.push('<tr class="agent-row"><td class="indent">' + esc(child.role || 'subagent') + ' — ' + esc((child.description || '').slice(0, 60)) +
        '<div class="meta">用时 ' + dur(child.durationMs) + ' · 轮次 ' + (child.turns || 0) +
  ...
  document.getElementById('runsfoot').textContent =
    ...
    ((runs.notes || []).length ? ' · ' + esc(runs.notes.join(' · ')) : '');
