...
export async function collectStatus(config) {
  ...
  // Who owns the bridge process is a setting rather than an observation, so it rides along with
  // the probe: 'on-demand' (default) starts nothing until something needs it, 'auto' starts it
  // while the plugin loads, 'off' never touches the process.
  status.bridge = {
    ...status.bridge,
    mode: config.bridgeMode ?? 'on-demand',
    script: config.bridgeScript ?? null,
  }
