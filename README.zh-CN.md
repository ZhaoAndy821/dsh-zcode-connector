...
**窗口放在哪、多大，也一样不影响。** 桥从不读窗口的位置和外部尺寸，它完全在渲染进程自己的坐标系里
工作：attach 时把这个坐标系钉住（`Emulation.setDeviceMetricsOverride`，1280×900），之后每次动作都重新读
目标元素的 `getBoundingClientRect()` 中心、用命中测试确认这个点确实落在目标元素上、再等 120ms 让它不再
移动才点下去。挪窗口、改大小、拖到屏幕边缘外 —— 下一个动作重新算一遍就行。实测（刚被拖到新位置的窗口，
同一个页面读两次）：

| 读数 | 输入框 | 发送键 | 视口 |
|---|---|---|---|
| 真实窗口布局（桥未动作时） | (345, 689) 802×40 | (1119, 741) | 1244×802 |
| 桥钉住的画布（attach 之后） | (345, 787) 838×40 | (1155, 839) | 1280×900 |

唯一的几何要求是：输入框和它的发送键要**在视口内**。窗口窄到把它们裁掉时，桥会明确报"定位失败"，
而不是瞎点。1244 宽时发送键右边缘在 x=1147，桥钉住的 1280 画布上是 x=1183。
...
2. **窗口位置和大小随便** —— 桥从不读它们，每次动作都在渲染进程自己那张被钉住的画布上重算元素中心。唯一的几何要求是输入框和发送键留在视口内；窄到裁掉它们时报"定位失败"，不会瞎点。
